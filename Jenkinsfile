pipeline {
    agent { label 'small' }
    environment {
      imagename_dev = "registry-gitlab.indocresearch.org/pilot/service_download"
      imagename_staging = "registry-gitlab.indocresearch.org/pilot/service_download"
    ...
    stage('Git clone for dev') {
        ...
        steps{
          script {
          git branch: "k8s-dev",
              url: 'https://git.indocresearch.org/pilot/service_download.git',
              credentialsId: 'lzhao'
            }
        }
    }
        ...
        script {
            withCredentials([usernamePassword(credentialsId:'readonly', usernameVariable: 'PIP_USERNAME', passwordVariable: 'PIP_PASSWORD')]) {
              docker.withRegistry('https://registry-gitlab.indocresearch.org', registryCredential) {
                  customImage = docker.build("registry-gitlab.indocresearch.org/pilot/service_download:$commit", "--build-arg pip_username=${PIP_USERNAME} --build-arg pip_password=${PIP_PASSWORD} --add-host git.indocresearch.org:10.4.3.151 .")
                  customImage.push()
              }
            }
        }
    ...
    stage('DEV Deploy') {
      ...
      steps{
        build(job: "/VRE-IaC/UpdateAppVersion", parameters: [
          ...
          [$class: 'StringParameterValue', name: 'TARGET_RELEASE', value: 'download' ],
